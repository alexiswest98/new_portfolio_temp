...
const ProjectCard = ({ isMobile, name, description, tags, image, source_code_link, github_link }) => {
  return (
    <div className='proj-card bg-hero-pattern3 bg-cover bg-no-repeat bg-center flex justify-center items-center gap-5'>
      <div className='projimage w-2/3'>
        <img src={image} alt='project preview'></img>
      </div>
      <div className='proj-descr bg-white/5 backdrop-blur-sm rounded-lg p-2 w-fit h-fit'>
        <div className='mt-6'>
          <h3 className={`text-white font-bold ${isMobile ? 'text-[25px]' : 'text-[30px]' }`}>{name}</h3>
          <p className={`mt-2 text-white ${isMobile ? 'text-[11px]' : 'text-[15px]'}`}>{description}</p>
      </div>
